...
class AIQualityAssuranceAgent:
    ...
    def _assess_confidence(self, output: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if score >= 0.9: assessment["confidence_level"] = "مرتفع جداً"
        elif score >= 0.75: assessment["confidence_level"] = "مرتفع"
        elif score >= 0.6: assessment["confidence_level"] = "متوسط"
        elif score >= 0.4: assessment["confidence_level"] = "منخفض"
        else:
            assessment["confidence_level"] = "منخفض جداً"
            assessment["warnings"].append("درجة الثقة منخفضة جداً")
        
        if "data_quality" in output: assessment["factors"].append(f"جودة البيانات: {output['data_quality']}")
        if "sample_size" in output: assessment["factors"].append(f"حجم العينة: {output['sample_size']}")
    ...
    def _calculate_accuracy_score(self, output: Dict[str, Any]) -> float:
        """حساب درجة الدقة"""
        score = 1.0
        if "errors" in output and output["errors"]: score -= len(output["errors"]) * 0.1
        if "warnings" in output and output["warnings"]: score -= len(output["warnings"]) * 0.05
    ...
    def _extract_numbers(self, data: Any) -> List[float]:
        ...
        numbers = []
        if isinstance(data, (int, float)): numbers.append(float(data))
        elif isinstance(data, dict):
            for value in data.values(): numbers.extend(self._extract_numbers(value))
        elif isinstance(data, list):
            for item in data: numbers.extend(self._extract_numbers(item))
    ...
    def _extract_text_fields(self, data: Any) -> List[str]:
        ...
        texts = []
        if isinstance(data, str): texts.append(data)
        elif isinstance(data, dict):
            for value in data.values(): texts.extend(self._extract_text_fields(value))
        elif isinstance(data, list):
            for item in data: texts.extend(self._extract_text_fields(item))
        return texts
    ...
    def _generate_recommendations(self, review_result: Dict[str, Any]) -> List[str]:
        ...
        for issue in review_result["issues_found"]:
            if issue["type"] == "schema_error": recommendations.append("مراجعة هيكل مخرجات الوكيل وإضافة الحقول المفقودة")
            elif issue["type"] == "consistency_error": recommendations.append("إجراء مراجعة يدوية للتناقضات المكتشفة")
            elif issue["type"] == "hallucination_warning": recommendations.append("تحسين دقة نموذج الذكاء الاصطناعي أو زيادة بيانات التدريب")
            elif issue["type"] == "low_confidence": recommendations.append("جمع بيانات إضافية أو تحسين خوارزميات التحليل")
        if not recommendations: recommendations.append("الجودة مقبولة - استمر في المراقبة الدورية")
    ...
    def _calculate_overall_quality(self, review_result: Dict[str, Any]) -> Tuple[str, bool]:
        ...
        if overall_score >= 0.9: return "ممتاز", True
        elif overall_score >= 0.75: return "جيد", True
        elif overall_score >= 0.6: return "مقبول", True
        elif overall_score >= 0.4: return "ضعيف", False
        else: return "رافض", False
    ...
    def _calculate_quality_distribution(self, reviews: List[Dict[str, Any]]) -> Dict[str, int]:
        ...
        for review in reviews:
            ...
            if quality in distribution: distribution[quality] += 1
        return distribution
