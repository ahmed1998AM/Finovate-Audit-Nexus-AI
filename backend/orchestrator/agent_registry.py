...
class AgentRegistry:
    ...
    def discover_agents(self) -> List[str]:
        ...
        for item in os.listdir(self.agents_path):
            ...
            # التحقق من وجود agent.py (أو enhanced_agent.py)
            if os.path.exists(os.path.join(item_path, 'agent.py')) or os.path.exists(os.path.join(item_path, 'enhanced_agent.py')):
    ...
    def load_agent(self, agent_name: str) -> Any:
        """
        تحميل وكيل محدد ديناميكياً (يفضل النسخة المحسّنة إن وجدت)
        ...
        """
        # 1) Try enhanced agent first (LLM-powered)
        try:
            module_path = f"agents.{agent_name}.enhanced_agent"
            module = importlib.import_module(module_path)
            # Look for Enhanced* class, skipping abstract base EnhancedAgent
            for attr_name in sorted(dir(module)):
                if attr_name == 'EnhancedAgent':
                    continue
                attr = getattr(module, attr_name)
                if isinstance(attr, type) and 'Enhanced' in attr_name and 'Agent' in attr_name:
                    agent_instance = attr()
                    logger.info(f"Loaded enhanced agent: {agent_name} ({attr.__name__})")
                    return agent_instance
        except Exception:
            pass  # Fall back to base below

        # 2) Fall back to base agent
        try:
            ...
            logger.info(f"Loaded base agent: {agent_name} ({agent_class.__name__})")
            return agent_instance
