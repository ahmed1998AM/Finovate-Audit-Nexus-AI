...
def _get_chief_agent():
    ...
    return get_chief_agent()

def _build_erp_connector():
    from connectors.sap_connector.connector import SAPConnectionConfig, SAPErpConnector
    config = SAPConnectionConfig(
        host=_os.getenv("SAP_HOST", "localhost"),
        system_number=_os.getenv("SAP_SYSTEM_NUMBER", "00"),
        client=_os.getenv("SAP_CLIENT", "100"),
        username=_os.getenv("SAP_USERNAME", ""),
        password=_os.getenv("SAP_PASSWORD", "")
    )
...
class AuditService:
    ...
    def __init__(self, db_session=None):
        ...
        self.chief_agent = _get_chief_agent()
        self.active_projects = {}
        logger.info("AuditService initialized")

    async def _fetch_erp_data(self, company_code: str, fiscal_year: str):
        connector = _build_erp_connector()
        connector.connect()
        try:
            return {
    ...
    def _save_analysis_results(self, session, analysis_results: Dict[str, Any], engagement_id: int):
    ...
    async def run_full_ai_audit(self, company_code: str, fiscal_year: str, engagement_id: int) -> Dict[str, Any]:
        ...
        try:
            erp_data = await self._fetch_erp_data(company_code, fiscal_year)
            self.db_manager.save_financial_data(session, erp_data["journal_entries"], "SAP")

            audit_data = {
                "journal_entries": erp_data["journal_entries"],
                "income_statement": {"current": erp_data["income_statement"]},
                "balance_sheet": erp_data["balance_sheet"],
                "entity_id": company_code,
                "fiscal_year": fiscal_year
            }
            analysis_results = await self.chief_agent.run_audit_workflow(audit_data)
            self._save_analysis_results(session, analysis_results, engagement_id)
