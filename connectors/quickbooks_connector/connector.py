...
# from authlib.integrations.requests_client import OAuth2Session  # unused; uses raw requests
...
class QuickBooksConnector(BaseERPConnector):
    ...
    def get_company_info(self) -> Dict:
        ...
        try:
            ...
            logger.error("خطأ في جلب معلومات الشركة: %s", e)
    ...
    def connect(self) -> bool:
        """إنشاء الاتصال بـ QuickBooks"""
        try:
            self._refresh_token()
            self._connected = True
            return True
        except Exception as e:
            logger.error("QuickBooks connection failed: %s", e)
            self._connected = False
            return False
