...
class ZohoBooksConnector(BaseERPConnector):
    ...
    def _get_access_token(self) -> str:
        ...
        try:
            ...
            logger.error("خطأ في الحصول على رمز الوصول: %s", e)
    ...
    def get_vendors(
        ...
        try:
            ...
            logger.error("خطأ في جلب البائعين: %s", e)
            return []
    ...
    def connect(self) -> bool:
        """إنشاء الاتصال بـ Zoho Books"""
        try:
            token = self._get_access_token()
            if token:
                self._connected = True
                return True
            logger.warning("Zoho connection failed: no access token")
            self._connected = False
            return False
        except Exception as e:
            logger.error("Zoho connection failed: %s", e)
            self._connected = False
            return False
